...
5. A Spring Data native `@Query` returning an interface projection cannot auto-convert a raw JDBC `byte[]` (MySQL `BINARY(16)`) column to a `UUID` getter — `ProjectingMethodInterceptor` throws `UnsupportedOperationException` at call time, never at query-definition time. Mockito-only tests can never catch this; it only surfaces against a real database. The fix is to express the same query in JPQL (using an ad-hoc JPA 2.1 `JOIN Entity2 e2 ON e2.x = e1.y` between otherwise-unassociated entities when there's no mapped relationship) so Hibernate's own type system performs the `UUID`/`Instant` conversions before the projection factory ever sees the row.
