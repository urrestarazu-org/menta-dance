...
Before consulting Billing, Virtual MUST evaluate the requested lesson in this order. The first matching rule MUST grant access without authentication:

1. `lesson.isFree == true`;
2. the lesson's module has `isPreview == true`;
3. the lesson's course belongs to no plan.

Only when none of those rules grants access MAY Virtual require an authenticated student and resolve entitlement through the shared Billing port. The BFF and every browser client MUST NOT make this authorization decision.
...
- GIVEN an anonymous visitor requests a lesson with `isFree = false` in a module with `isPreview = true`
- WHEN Virtual resolves its access
...
#### Scenario: A course absent from every plan is public

- GIVEN an anonymous visitor requests a non-free, non-preview lesson whose course belongs to no plan
- WHEN Virtual resolves its access
- THEN it grants the request without authentication or Billing entitlement
...
Lesson detail and `GET /api/v1/virtual/lessons/{lessonId}/stream` MUST apply the same access cascade and entitlement semantics. Lesson detail MUST include explicit metadata that distinguishes public/preview access from `requiresSubscription`; it MUST never embed a streaming URL. A material endpoint does not yet exist and is out of scope.

The OpenAPI contract and versioned Bruno collection MUST document and assert the protected-path `403 application/problem+json` contract, including that denial yields no stream URL.
