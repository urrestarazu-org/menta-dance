...
- [x] 5.1 RED `SubscriptionExpiryReconcilerTest#delegatesPerId`: `tick()` calls the injected `SubscriptionExpiryWorker.expireOne` once per id returned by `findExpirableIds` [S6, S7]
- [x] 5.2 RED `SubscriptionExpiryReconcilerTest#batchSurvivesFailure`: a worker throwing on row 1 still processes rows 2..n (per-row `try/catch`, A6) [S6, S7]
- [x] 5.3 GREEN `billing/infrastructure/scheduling/SubscriptionExpiryReconciler.java`: `@Component` **on the class** plus `@ConditionalOnProperty(name = "billing.subscription.expiry.enabled", havingValue = "true", matchIfMissing = true)` **on the class, not on `tick()`** — Spring evaluates that condition only on a component/configuration class or a `@Bean` method, so on a plain method it is inert and the job would run unconditionally (A16); `@Scheduled(fixedRateString)` on `tick()`, **no** `@Transactional`, per-row `try/catch` + `log.warn`
- [x] 5.4 RED `SubscriptionExpiryWorkerTest#skipsSaveOnNoop`: `expireOne` does not call `save` when `expire()` returns the same instance [S8]
- [x] 5.5 GREEN `billing/infrastructure/scheduling/SubscriptionExpiryWorker.java`: separate class registered by `@Component` **only**, `@Transactional(propagation = REQUIRES_NEW) expireOne(UUID)` (A6/A6b)
- [x] 5.6 RED `SubscriptionExceptionHandlerTest#optimisticLock`: `ObjectOptimisticLockingFailureException` → `409 SUBSCRIPTION_CONFLICT` (A14)
- [x] 5.7 GREEN `SubscriptionExceptionHandler.java`: map `ObjectOptimisticLockingFailureException` → `409`
- [x] 5.8 RED (integration) `SubscriptionExpirySweepIntegrationTest#sweepBeansAreScannedOnce` (A6b): the **running** context exposes exactly one `SubscriptionExpiryReconciler` bean and exactly one `SubscriptionExpiryWorker` bean — a duplicate registration would mean two concurrent `tick()`s over the same batch. A `@TestPropertySource(billing.subscription.expiry.enabled=false)` variant asserts the reconciler bean is **absent**, proving A16's off switch is real. **Deviation**: implemented as a separate top-level class `SubscriptionExpiryDisabledIntegrationTest` (same package) rather than a `@Nested` class inside `SubscriptionExpirySweepIntegrationTest`, since Spring's context cache keys on the merged config (incl. `@TestPropertySource`) and a distinct top-level class gives an unambiguous, independently cached context without relying on `@Nested` + `@SpringBootTest` composition. Both classes live in the `@SpringBootTest` layer, not `BillingConfigurationTest`, per the original rationale
- [x] 5.9 GREEN `application.yml`: defaults for `billing.subscription.expiry.enabled`, `rate-ms` and the batch size. **No `@Bean` is added to `BillingConfiguration` for the reconciler or the worker** — `@Component` is their single registration path, matching `WebhookInboxReconciler`/`WebhookVerificationWorker`, which appear nowhere in that class (verified)
- [x] 5.10 RED (integration) `SubscriptionExpirySweepIntegrationTest#stalePaidAndTrial`: real `ACTIVE → EXPIRED` for one stale `PAID` and one stale `TRIAL` via `tick()` against Testcontainers MySQL [S6, S7]
- [x] 5.11 RED (integration) `SubscriptionExpirySweepIntegrationTest#untouched`: non-stale and non-`ACTIVE` (`CANCELLED`/already-`EXPIRED`) rows keep `status`/`endDate` unchanged [S8]
- [x] 5.12 RED (integration) `SubscriptionExpirySweepIntegrationTest#batchResilience`: a row that throws does not stop the batch; expired row keeps its course snapshot (`save` → `replaceCourses` trap). Implemented via a real, naturally-occurring failure (an A17-violating malformed row: `PAID` with `payment_id IS NULL`) rather than a mock, so the per-row `catch` is proven against a genuine mapping exception
- [x] 5.13 GREEN `api/app/src/test/.../integration/billing/SubscriptionExpirySweepIntegrationTest.java` implementing 5.8 (partially — see 5.8's deviation), 5.10–5.12
- [x] 5.14 RED (integration) `SubscriptionOptimisticLockingIntegrationTest`: load same subscription twice, save one copy, save the stale copy → `ObjectOptimisticLockingFailureException`; a cancellation committed between the sweep's read/write leaves the cancellation audit intact (A14)
- [x] 5.15 GREEN `api/app/src/test/.../integration/billing/SubscriptionOptimisticLockingIntegrationTest.java` implementing 5.14
- [x] 5.16 GREEN **A14 contract widening on already-delivered endpoints** — `SubscriptionExceptionHandler` is a `@RestControllerAdvice(annotations = SubscriptionEndpoint.class)`, and `@SubscriptionEndpoint` is carried by `SubscriptionController` and `SubscriptionAdminController` (verified), so the new `409 SUBSCRIPTION_CONFLICT` becomes a reachable response on routes shipped before this change. Documented in `api/openapi/billing-v1.yaml` for `POST /api/v1/subscriptions`, `DELETE /api/v1/subscriptions/me` and `DELETE /api/v1/admin/billing/subscriptions/{subscriptionId}` (the #130 cancellation routes) as well as `POST .../trial` (added a second `409` example alongside the pre-existing `SUBSCRIPTION_ALREADY_ACTIVE` one), and noted in the docs block of `bruno/API - Direct/billing/Cancel Own Subscription.bru`, `Cancel Subscription (Admin).bru` and `Create Subscription Checkout.bru`. **PR description must call this out**: this is an observable contract change to already-merged functionality, not a trial-only addition
