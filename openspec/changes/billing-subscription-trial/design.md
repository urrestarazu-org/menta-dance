...
| `billing/domain/model/Subscription.java` | Modify | 16-arg constructor + 17-arg rehydration constructor (`version`, A14); `trial(...)`; `expire(Instant)` with both A13 guards; `Optional<PaymentId> getPaymentId()`; `getType()`, `getTrialGrant()`, `getVersion()`; `copy(...)` carries type, grant and version. |
...
| `billing/infrastructure/config/BillingConfiguration.java` | Modify | New `@Bean`s (use case with `UserExistencePort`, reconciler, worker). |
| `billing/db/migration/V18__billing_subscription_trial.sql` | Create | `MODIFY payment_id … NULL`, `type`, 4 grant columns, `version BIGINT NOT NULL DEFAULT 0`, sweep index. Last existing is **V17** (confirmed). |
...
| `api/openapi/billing-v1.yaml`, `bruno/API - Direct/billing/Assign Trial Subscription.bru` | Modify/Create | Route contract incl. the D8 `404`. |
...
public TrialAssignmentResult assign(AssignTrialCommand command) {
    ...
    return TrialAssignmentResult.from(subscriptionRepository.saveNewSubscription(Subscription.trial(
        ...
        plan.getCourseIds(), new TrialGrant(now, command.actingUserId(), command.reason(), command.days())
    )));
}
...
// api/billing — the sweep (A6). tick() is deliberately NOT @Transactional.
// A16: the class carries @ConditionalOnProperty so the job can actually be switched off.
...
| Wiring | `BillingConfigurationTest` asserts the trial use-case bean is built with the injected `UserExistencePort`, and that reconciler and worker are two distinct beans | Existing config-test convention |
...
V18 is additive plus one relaxation (`MODIFY payment_id BINARY(16) NULL`) and one new `version BIGINT NOT NULL DEFAULT 0` (A14); no data rewrite, no back-fill — existing rows start at version `0`, which is exactly what a freshly mapped entity carries. D8 needs **no** migration and validates no existing row. **A16 — the sweep needs a real off switch.** `billing.subscription.expiry.rate-ms` only changes the *interval*; a `@Scheduled(fixedRateString)` job cannot be disabled by any value of that property, so the deploy note below would otherwise describe a control that does not exist. `SubscriptionExpiryReconciler` therefore carries `@ConditionalOnProperty(name = "billing.subscription.expiry.enabled", havingValue = "true", matchIfMissing = true)`: on by default (so no environment silently loses expiry), off by an explicit `false`. Rejected: injecting a boolean and returning early from `tick()` — the bean and its schedule would still exist, which is the kind of half-off state that makes an incident harder to reason about. **Deploy note:** the first tick expires every already-past-`endDate` row that has never been expired — verify that backlog's size before rollout, and set `billing.subscription.expiry.enabled=false` if it must be drained deliberately rather than in one tick.
...
|---|---|---|---|
| 1 | Domain (`SubscriptionType`, `TrialGrant`, both constructors, `trial`, `expire` + A13 guards, `version`), V18, persistence + mapper + full `getPaymentId()` ripple, `saveNewSubscription` (A12) + `findExpirableIds` (A2) on port and adapter | ~620 | Yes — compiles green, no new behavior exposed |
...
| 5 | `SubscriptionExpiryReconciler` + `SubscriptionExpiryWorker` + config + `409` optimistic-lock mapping + sweep and locking integration tests | ~360 | Yes — closes escenario 5 |

Chain: `1 → 2 → 3 → 4 → 5`, each targeting the previous slice's branch. Total ~2020 lines (up ~240 from the pre-correction estimate: A12's repository method, A6's worker bean, A14's version plumbing and A13's guard, plus their tests). Every slice is under the 800-line budget. Slice 1 is the largest and is deliberately not split further: `Subscription`, `SubscriptionJpaMapper` and V18 are edited once for the type, the nullable payment and the version together — splitting them would mean two migrations on the same table and a mid-chain commit where the mapper reads a column the aggregate cannot carry.

## Open Questions

- [ ] **Spec wording (A15)**: the spec says "target student" in 7 places, but the implemented check is existence-only, so an admin can technically grant a trial to another admin or an instructor. The spec should read "target user". This is editorial — **no behavior changes either way** — so the spec file is left untouched here; the orchestrator decides whether it is worth another `sdd-spec` pass.
